...
int main()
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		//把获取uniform矩阵位置的代码放在了循环的外面并没有什么影响.
		...
		如果当前的片段在其它片段之后，它将会被丢弃，否则将会覆盖。这个过程称为深度测试(Depth Testing)，它是由OpenGL自动完成的。
		...
		这个功能会一直保持启用/禁用状态，直到另一个调用来禁用/启用它。现在我们想启用深度测试，需要开启GL_DEPTH_TEST：
