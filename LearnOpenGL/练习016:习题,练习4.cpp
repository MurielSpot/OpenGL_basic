...
片段着色器要改,加一个uniform变量:
uniform float mixValue=0.3;

代码正文添加了一个变量:
//控制物体透明度,对应片段着色器中的transparency.
// stores how much we're seeing of either texture
float mixValue = 0.2f;

因为uniform每次按过按键后都会改变,所以告诉着色器要设置它的值时,要放在渲染循环里:
...
void processInput(GLFWwindow *window)
{
  ...
  //增加了下面的部分.
	...
	{
		//我设置成0.1f时,按一下就完全切换了两个图片,透明度的值变化得非常快.所以还是调成0.01f吧.
